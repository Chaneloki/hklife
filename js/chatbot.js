...
function buildActiveMessageEvaluationPrompt(character, activeMessage, playerFreeReply, s) {
  ...
  return [
    ...
    "relationshipDelta 唔係「稱讚就加、開玩笑就減」；要按 NPC 性格、目前訊息內容、玩家語氣有冇尊重對方重視嘅東西來判斷。",
    "空泛討好例如「你好叻」「你講咩都啱」通常係 0；友善玩笑對貪玩／愛幻想角色可以係 +1，但對敏感、安靜、競爭壓力大嘅角色可能係 0 或 -1。",
    "直接羞辱、嘲笑對方重視嘅物件或感受、逼問太私人嘅事，通常係 -1。",
    "請同時回傳 relationshipDecision，intent 只可以係 support_npc / ask_question / playful_join / teasing_friendly / teasing_mean / practical_solution / set_boundary / avoid_or_silent / challenge_npc / dismiss_npc / compare_or_compete / share_personal_memory / seek_adult_help。",
    "",
    `NPC: ${character.name}`,
    ...
    '  "relationshipDelta": -1,',
    '  "relationshipDecision": { "intent": "teasing_friendly", "tone": "playful", "reasonCode": "friendly_teasing_accepted" },',
...
export async function evaluateActiveMessageReplyWithApi(character, activeMessage, playerFreeReply, s, options = {}) {
  ...
  return {
    ...
    evaluation: {
      ...
      relationshipDecision: parsed.relationshipDecision && typeof parsed.relationshipDecision === "object" ? parsed.relationshipDecision : null,
...
function buildOpeningEventEvaluationPrompt(context, playerFreeReply, s) {
  ...
  return [
    ...
    "每次都必須回傳至少一個 statusDelta，至少一個 skillExpDelta。",
    "如果目前有 NPC，relationshipDelta 必須至少有一項；如果沒有 NPC，relationshipDelta 可以是空陣列。",
    "即使玩家只是觀察、沉默、避開或輕輕回應，也要按情境給低幅度變化，例如理智值 +1、閱讀 +1、closeness +1 或 misunderstanding +1。",
    "每個數值 amount 必須是整數，最少 -2，最多 +2。不要超過這個範圍。statusDelta／skillExpDelta／relationshipDelta 只係俾唔夠信心時嘅備用參考，實際套用嘅數值由遊戲另外決定，唔會照抄你呢度嘅數字。",
    "relationshipDelta 只可以影響當前 NPC，不可以影響其他角色。",
    "relationshipDelta 唔係「稱讚就加、開玩笑就減」；要按 NPC 性格、目前 authored event 內容、玩家語氣有冇尊重對方重視嘅東西來判斷。",
    "空泛討好例如「你好叻」「你講咩都啱」通常係 0；友善玩笑對家朗／柏宇／活潑鄰居可以係正面，但對穎心／梓軒／安靜鄰居可能只係 0，甚至踩中不安就 -1。",
    "玩家用小一語氣提出界線、慢慢做、問清楚、尊重物件，可以係正面；直接羞辱、嘲笑對方重視嘅物件或感受、逼問太私人嘅事，係負面。",
    "你必須將玩家嘅自由輸入歸類做以下其中一個 resolvedIntent（唔可以自創新分類）：",
    "support_npc / ask_question / join_activity / slow_down / set_boundary / avoid_or_silent / joke_playfully / practical_solution / compare_or_compete / dismiss_npc / seek_adult_help / share_personal_memory",
    "relationshipDecision.intent 可以用更細分類：support_npc / ask_question / playful_join / teasing_friendly / teasing_mean / practical_solution / set_boundary / avoid_or_silent / challenge_npc / dismiss_npc / compare_or_compete / share_personal_memory / seek_adult_help。",
    "confidence 係 0 到 1 之間嘅數字，代表玩家輸入同你揀嘅 reviewAnchorChoiceId 有幾接近：非常接近（幾乎係同一句意思）填 0.75 以上；合理但唔完全對應填 0.45–0.75；太模糊（例如「嗯」「ok」「唔知」）或答非所問填 0.45 以下。",
    "如果玩家回答傷害、嘲笑、否定或羞辱 NPC，resolvedIntent 必須係 dismiss_npc。",
    "如果玩家輸入語氣過度成人化、超出小一學生應有嘅語氣，或者出現呢個世界觀不應該有嘅內容，resultText／resultDialogue 要 normalize 成小一學生可以講嘅短句，唔可以原句照抄。",
    "resultText／resultDialogue 只可以描述呢一次小反應，不可以新增任何長期系統結果、新角色、新地點或新 event。",
    "",
